...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  sub_dirs = sorted(x for x in tf.gfile.Walk(image_dir))
  ...
  for sub_dir in sub_dirs:
    ...
    file_list = []
    dir_name = os.path.basename(sub_dir[0])
    if dir_name == image_dir:
      continue
    tf.logging.info("Looking for images in '" + dir_name + "'")
    for f in sub_dir[2]:
      if os.path.splitext(f)[1] in extensions:
        file_list.append(os.path.join(image_dir, dir_name, f))
    if not file_list:
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      type=str,
      default=(
          'http://tfhub.dev/google/imagenet/inception_v3/feature_vector/1'),
      help="""\
